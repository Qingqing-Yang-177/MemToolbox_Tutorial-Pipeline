...
% The toolbox also provides two helper functions that provide an alternative route to fitting 
% data from multiple participants. The FitMultipleSubjects_MAP function computes the 
% MAP estimate for each subject and then also calculates means and standard errors 
% for each parameter for you; there is a similar FitMultipleSubject_MLE function that 
% calculates maximum likelihood parameters rather than maximum a posterior parameters
% (i.e., it does not consider the prior at all).
